    ...
    for season in range(1,5,1) : 
        ...
        for table_num in range(1, len(table_list),1) :     
            ...
            if text_list != [] : 
                ...
                newdata = {'year':int(volume)+1976,'volumn':volume,'season':season,'table':table_num,'keyword':'nan','url':url_target }
                df =  df.append(newdata, ignore_index=True)

df.to_excel('raw data_220319.xlsx', encoding='utf-8-sig', index = None, header=True)
