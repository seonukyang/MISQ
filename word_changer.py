    ...
    for k in range(0, len(category['keyword_before']),1):
        ...
        if text_filter == test_word : 
            for i in range(0, len(keyword), 1): 
                if str(keyword[i]).lower().replace(' ','') == test_word :               
                    del keyword[i]
...
def method_fil_same(text_filter, text) :
...
#method_before과 일치여부
def method_before_same(text_filter, text) : 
...
def theory_fil_same(text_filter, text) :
...
#theory_before과 일치여부
def theory_before_same(text_filter, text) : 
...
for year in year_list : 
    ...
    # for i in range(62,63,1) :
        ...
        # for j in range(0, 3,1) :
            ...
            keyword.append(text_list[j])
            if country_del(text_filter, text_list[j]) == True:
                if keyword_before_same(text_filter) == True : 
                    if method_before_same(text_filter, text_list[j]) == True :
                        if method_fil_same(text_filter, text_list[j]) == True : 
                            if theory_before_same(text_filter, text_list[j]) == True : 
                                theory_fil_same(text_filter, text_list[j])                
    ...
    df_category.to_csv('result'+year+'_'+today+'.csv', encoding='utf-8-sig')
